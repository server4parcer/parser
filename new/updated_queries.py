"""
Database Queries - Enhanced SQL queries for new data model fields
"""
...
# Queries for the URL table
class UrlQueries:
    """SQL queries for URL table."""
    ...
    def update(
        ...
        # If no fields to update, return query to get URL
...
# Queries for the booking_data table
class BookingQueries:
    """SQL queries for booking_data table."""
    ...
    def get_all(
        ...
        """Query to get all booking data with new filters."""
        query = f"""
    ...
    def count(
        ...
        """Query to count booking data with new filters."""
        query = f"""
            SELECT COUNT(*)
            FROM {BOOKING_TABLE} b
            JOIN {URL_TABLE} u ON b.url_id = u.id
    ...
    def get_by_id(booking_id: int) -> Tuple[str, List[Any]]:
        """Query to get booking data by ID."""
        query = f"""
            SELECT b.id, u.url, b.date, b.time, b.price, b.provider, b.seat_number, 
                   b.location_name, b.court_type, b.time_category, b.duration, 
                   b.review_count, b.prepayment_required, b.raw_venue_data,
    ...
    def create(
        ...
        """Query to create booking data with new fields."""
        query = f"""
    ...
    def update(
        ...
        """Query to update booking data with new fields."""
        update_fields = []
        params = []
        ...
        # If no fields to update, return query to get booking data
        if not update_fields:
    ...
    def delete_by_url_id(url_id: int) -> Tuple[str, List[Any]]:
        """Query to delete booking data by URL ID."""
    ...
    def delete_old_data(days: int) -> Tuple[str, List[Any]]:
        """Query to delete old booking data."""
    ...
    def get_price_ranges_by_court_type() -> Tuple[str, List[Any]]:
        """Query to get price ranges by court type."""
        query = f"""
            SELECT 
                court_type,
    ...
    def get_price_comparison_by_time_category() -> Tuple[str, List[Any]]:
        """Query to get price comparison by time category."""
        query = f"""
            SELECT 
                court_type,
                time_category,
    ...
    def get_availability_by_location() -> Tuple[str, List[Any]]:
        """Query to get availability by location."""
        query = f"""
            SELECT 
                location_name,
                date,
    ...
    def get_court_types_by_venue() -> Tuple[str, List[Any]]:
        """Query to get court types by venue."""
        query = f"""
            SELECT 
                u.url,
                b.location_name,
...
# Queries for the new price_history table
class PriceHistoryQueries:
    """SQL queries for price_history table."""
    
    @staticmethod
    def create(booking_data_id: int, price: str) -> Tuple[str, List[Any]]:
        """Query to create price history record."""
        query = """
    ...
    def get_by_booking_data_id(booking_data_id: int) -> Tuple[str, List[Any]]:
        """Query to get price history by booking data ID."""
        query = """
            SELECT *
            FROM price_history
    ...
    def get_price_changes(days: int = 30) -> Tuple[str, List[Any]]:
        """Query to get price changes over specified days."""
        query = f"""
...
# Queries for the new availability_analytics table
class AvailabilityAnalyticsQueries:
    """SQL queries for availability_analytics table."""
    ...
    def create(
        ...
        """Query to create availability analytics record."""
        query = """
    ...
    def get_availability_trends(days: int = 30) -> Tuple[str, List[Any]]:
        """Query to get availability trends over specified days."""
        query = """
            SELECT 
                aa.date,
                aa.time_slot,
